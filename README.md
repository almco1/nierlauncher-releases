...
| `SYSTEM WALLPAPER` | いまの背景を端末の壁紙にも焼き込む。ロック画面そのものは差し替えられませんが、背景が揃うと時計の色まで寄ります。`ロック画面に適用` / `ホーム壁紙にも適用` の 2 つのスイッチがあります |

`アクセント罫線` のスイッチは、**正方形に近い画面の端末でだけ**出ます（長辺 ÷ 短辺 ≦ 1.3。
Unihertz Titan 2 など）。ロック画面の時計の位置はアプリから知る方法が無く、縦長の端末では
狙った場所に引けないためです。
